...
from lib.recognizer import load_action_premodel,analyze_model
...
#load model
estimator = moset.load_pretrain_model('mobilenet_thin')
action_classifier = load_action_premodel('train/models.h5')
#------------------------------------------------------------------------
#calculate FPS
def FPS():
...
while cv.waitKey(1) != 13:
    ...
    if not is_ret:
        ...
        #analyze action in every frame
        frame = analyze_model(pose, action_classifier)
